...
#ifdef __APPLE__
	#define DEF_COMPILER "clang++"
#else
	#define DEF_COMPILER "g++"
#endif
...
int main(int argc, char** argv){
	...
	if (first_arg == "-h" || first_arg == "-H" || first_arg == "-HELP" || first_arg == "-help" || first_arg == "-Help"){
		...
		cout << "\t//CXCOMPILE " << string(DEF_COMPILER) << " -o output testprogram.cpp" << endl;
	...
	if (compiler_commands.size() < 1){
		compiler_commands.push_back(string(DEF_COMPILER) + " -o " + filename + " " + filename_w_ext);
	}
